...
async function fetchVideoDetails(videoIds: string[]): Promise<YouTubeVideo[]> {
  if (videoIds.length === 0) {
    console.warn("⚠️ No video IDs provided to fetchVideoDetails.");
    return [];
  }

  let allVideos: YouTubeVideo[] = [];
  const batchSize = 1; // YouTube API limit per request

  try {
    for (let i = 0; i < videoIds.length; i += batchSize) {
      const batch = videoIds.slice(i, i + batchSize); // 🔹 Process 50 IDs at a time
      const response = await youtube.videos.list({
        part: ["snippet", "statistics"],
        id: batch.join(","), // ✅ Convert array to comma-separated string
      });
