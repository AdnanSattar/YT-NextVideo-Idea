...
export async function kickoffIdeaGeneration(): Promise<void> {
  ...
  comments: comments.map(comment => ({
    video_title: comment.title,  // Ensure correct key names
  ...
  try {
    ...
    const apiUrl = process.env.NEXT_PUBLIC_CREWAI_URL?.replace(/\/$/, '') || process.env.CREWAI_URL;

    const kickoffResponse = await fetch(`${apiUrl}/kickoff/`, {
      method: "POST",
      headers: {
