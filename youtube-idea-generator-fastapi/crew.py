...
from SearchYouTubeTool import YoutubeVideoSearchAndDetailsTool
...
class YoutubeIdeaGeneratorCrew:
    ...
    def video_idea_generator_agent(self) -> Agent:
        return Agent(config=self.agents_config["video_idea_generator_agent"], verbose=True)
    ...
    def filter_comments_task(self) -> Task:
        return Task(config=self.tasks_config["filter_comments_task"])

    @task
    def generate_video_ideas_task(self) -> Task:
        return Task(config=self.tasks_config["generate_video_ideas_task"])

    @task
    def research_video_ideas_task(self) -> Task:
        return Task(config=self.tasks_config["research_video_ideas_task"])
    ...
    def crew(self) -> Crew:
        ...
        return Crew(
            ...
            verbose=True,
            # To use a hierarchical process, uncomment the line below:
